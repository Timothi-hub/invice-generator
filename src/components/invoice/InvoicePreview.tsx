...
const InvoicePreview = forwardRef<HTMLDivElement, InvoicePreviewProps>(
  ({ invoice, profile, showExpensesProfit = true }, ref) => {
    ...
    return (
      <div ref={ref} className="bg-white w-full max-w-[210mm] mx-auto shadow-xl" style={{ minHeight: '297mm' }}>
        ...
        <div className="invoice-header-gradient text-white p-6 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="flex items-center gap-4">
              {profile.logoUrl ? (
                <img src={profile.logoUrl} alt="Logo" className="w-16 h-16 object-contain rounded-lg bg-white/10 p-2" />
              ) : (
        ...
        <div className="p-6 flex justify-between">
          ...
          <div className="w-20 h-20 opacity-20">
            <svg viewBox="0 0 100 100" fill="none">
              {[...Array(6)].map((_, i) => (
                <line key={i} x1={10 + i * 15} y1="0" x2={10 + i * 15} y2="100" stroke="currentColor" strokeWidth="2" className="text-invoice-accent" />
              ))}
            </svg>
          </div>
        </div>
        ...
        <div className="invoice-footer-gradient text-white p-6 mt-8 relative">
          <div className="absolute right-8 -top-10 w-24 h-24 rounded-full invoice-accent-circle"></div>
